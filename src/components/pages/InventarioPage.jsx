...
export default function InventarioPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    status: '',
    quantity: '',
    registerDate: new Date().toISOString().split('T')[0], // Formato YYYY-MM-DD
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      fetchInventory(); // Recargar el inventario desde Firestore
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este material?')) {
      try {
        ...
        fetchInventory(); // Recargar el inventario
      } catch (error) {
                ...
                <div className="text-center py-12">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {searchTerm 
                      ...
                      : 'Empieza agregando nuevos materiales al inventario.'
                    }
                  </p>
                </div>
              ) : (
